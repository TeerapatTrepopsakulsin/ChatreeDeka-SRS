...
\usepackage{array}     % For better column control (table)
\usepackage{float}     % Allows [H] figure placement
\usepackage{longtable} % Multi-page tables (use cases)
\usepackage{needspace} % Prevent awkward page breaks before headings/blocks
\usepackage{pgfgantt}  % Gantt charts for project schedule
...
\def \srsTitle{ChatreeDeka}
\def \srsAuthorOne{Teerapat Trepopsakulsin}
\def \srsAuthorTwo{Pichapop Rodduayboon}
\def \srsAuthorThree{}
\def \srsAcademicYear{2026}

\def \srsAdvisorName{Paruj Ratanaworabhan}
